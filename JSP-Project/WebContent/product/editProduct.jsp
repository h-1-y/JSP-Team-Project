...
<div class="bg-light py-3">
      <div class="container">
        <div class="row">
          <div class="col-md-12 mb-0"><a href="${pageContext.request.contextPath }/home.jsp">Home</a> <span class="mx-2 mb-0">/</span> <strong class="text-black">Product Edit</strong></div>
        </div>
      </div>
    </div>
              ...
              <div class="col-sm-6 col-lg-4 mb-4" data-aos="fade-up">
                <div class="block-4 text-center border" style="width:255px;height:440px;">
                  <figure class="block-4-image"">
                    <a href="${pageContext.request.contextPath }/product/updateProduct.jsp?p_id=<%=rs.getString("p_id")%>"><img style="width:253px;height:253px;" src="${pageContext.request.contextPath }/images/<%=rs.getString("p_fileName")%>" alt="Image placeholder" class="img-fluid"></a>
                  </figure>
                  <div class="block-4-text p-4">
                    <h3><a href="${pageContext.request.contextPath }/product/updateProduct.jsp?p_id=<%=rs.getString("p_id")%>"><%=rs.getString("p_name")%></a></h3>
                    <p class="mb-0"><%=rs.getString("p_description")%></p>	
                    <p>
				 <%
			    	if(edit.equals("update")){
				 %>
				 <a href="${pageContext.request.contextPath }/product/updateProduct.jsp?p_id=<%=rs.getString("p_id")%>" 
				 class="buy-now btn btn-sm btn-primary" 
				 style="background-color:#28a745;border-color:#28a745;" role="button">Edit</a>
				  <%}else if(edit.equals("delete")){ %>
				 <a href="#" onclick="deleteConfirm('<%=rs.getString("p_id")%>')"
				 class="buy-now btn btn-sm btn-primary" 
				 style="background-color:#dc3545;border-color:#dc3545;" role="button">Delete</a>
				 <%} %>
				   </p>
                  </div>
                </div>
...
function deleteConfirm(id){
	if(confirm("Are you really sure to delete "+id+"???")==true){
